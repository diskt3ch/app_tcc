...
O **Raízes do Nordeste** é um sistema completo de ponta a ponta (E2E) para gestão de pedidos de uma rede de lanchonetes de culinária típica nordestina. Desenvolvido como **Trabalho de Conclusão de Curso (TCC)** para o Projeto Multidisciplinar, o sistema unifica múltiplos canais de venda (App Mobile, Totem de Autoatendimento e Balcão/PDV) e oferece uma gestão centralizada com painel administrativo em tempo real.

O projeto foi concebido a partir de um estudo de caso acadêmico, aplicando conceitos modernos de engenharia de software: **arquitetura serverless**, **pagamento desacoplado via webhook**, **geolocalização nativa** e **design system componentizado**.
...
| Funcionalidade | Descrição |
|---|---|
| 🔐 **Login Restrito** | Tela de autenticação com credenciais operacionais |
...
O projeto adota uma **arquitetura de microsserviços desacoplada e escalável**, utilizando o Firebase como plataforma serverless.
...
| **Vite** | 8.x | Build tool e dev server ultra-rápido |
| **TailwindCSS** | 3.4 | Framework CSS utility-first responsivo |
| **Geolocation API** | Web API | Captura de coordenadas GPS nativas |

### Backend

| Tecnologia | Versão | Finalidade |
|---|---|---|
| **Firebase Cloud Functions** | 4.x | Lógica serverless (Webhook de pagamento) |
| **Firebase Admin SDK** | 11.x | Acesso administrativo ao Firestore |
...
| Serviço | Finalidade |
|---|---|
| **Firebase Hosting** | CDN global para distribuição do SPA |
| **Cloud Firestore** | Banco NoSQL de documentos em tempo real |
| **Firestore Security Rules** | Controle de acesso granular por coleção |
...
> ⚠️ **Nota:** Essas credenciais são para fins de demonstração acadêmica. Em um ambiente de produção real, utilize autenticação segura (Firebase Auth, OAuth, etc.).
...
```javascript
// Regras de acesso granular por coleção
match /unidades/{unidadeId} {
  allow read: if true;         // ✅ Cardápio é público
}

match /produtos/{produtoId} {
  allow read: if true;         // ✅ Produtos são públicos
}

match /{document=**} {
  allow read, write: if request.auth != null;  // 🔐 Demais dados requerem autenticação
}
```
...
O desenvolvimento atendeu rigorosamente a todas as diretrizes do **Roteiro Prático do Projeto Multidisciplinar**:
...
Este projeto foi desenvolvido com suporte ativo de **Inteligência Artificial** (Antigravity e Stitch), utilizada como **Pair Programmer** — não como gerador automático de código. A documentação completa de uso de IA, incluindo prompts utilizados e aprendizados consolidados, está no arquivo [`Doc_TCC_Raizes_do_Nordeste.md`](Doc_TCC_Raizes_do_Nordeste.md).
